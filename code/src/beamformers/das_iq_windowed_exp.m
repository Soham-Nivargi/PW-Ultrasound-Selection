function image = das_iq_windowed_exp(scan,dataset,pw_indices, path_scan, path_pht, flag_simu, flag_display)
    ...
    win = tukeywin(floor(rows/2), 0.25) * tukeywin(cols, 0.25)';
    padded_win = padarray(win, [rows-floor(rows/4) cols-floor(cols/2)], 0);
    coordinates = {[193,153], [193, 457]};
    w = cell(1,2);
    for i=1:2
        w{i} = padded_win(rows-coordinates{i}(2):2*rows-coordinates{i}(2)-1, cols-coordinates{i}(1):2*cols-coordinates{i}(1)-1);
        w{i} = w{i} / max(w{i}(:));
    ...
    for f=1:length(pw_indices)
        ...
        for pw=pw_indices{f}
            ...
            figure();
            imshow(window);
            saveas(gcf, ['Results/uniform_experiment/yes_window/window_', num2str(j), '.jpg']);
            ...
            dynamic_range = 60;
            filename1 = ['Results/uniform_experiment/yes_window/iteration', num2str(j), '.jpg'];
    ...
    windowed_path_jpg = ['Results/uniform_experiment/yes_window/windowed', '.jpg'];
    close(wb);
    reg_us = us_image('DAS-IQ beamforming');
    reg_us.author = 'Alfonso Rodriguez-Molares <alfonso.r.molares@ntnu.no>';
    reg_us.affiliation = 'Norwegian University of Science and Technology (NTNU)';
    ...
    windowed_path = ['Results/uniform_experiment/yes_window/windowed', '.hdf5'];
    ...
    unwindowed_path_jpg = ['Results/uniform_experiment/yes_window/unwindowed', '.jpg'];
    ...
    unwindowed_path = ['Results/uniform_experiment/yes_window/unwindowed', '.hdf5'];
