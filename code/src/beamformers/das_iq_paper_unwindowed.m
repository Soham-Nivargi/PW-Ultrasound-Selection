function envelope_beamformed_data = das_iq_paper_unwindowed(scan,dataset,arr) %, path_reconstruted_img, path_image
    ...
    %-- reshape
    envelope_beamformed_data = reshape(beamformed_data,[numel(scan.z_axis) numel(scan.x_axis)]);
    ...
    % image.scan = scan;
    % image.number_plane_waves = length(arr); 
