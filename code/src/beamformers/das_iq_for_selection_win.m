function [beamform_next, beamform_comp] = das_iq_for_selection_win(scan,pht,dataset,pw_indice,beamform_curr,l)
    ...
    temp_us.scan = scan;
    temp_us.number_plane_waves = l;
    temp_us.data = abs(beamform_curr);
    ...
    contrast_init = tools.contrast_score(scan, pht, temp_us, l);


    for f=1:length(pw_indice) 
        contrast_reg = contrast_init;
        reg_image = beamform_curr;
        reg_image_comp = abs(beamform_curr);
        j = l;
        ...
        for pw=pw_indice{f}
            ...
            reg_image_comp = abs((reg_image_comp*j).*(2-window)+abs(envelope_temp_data).*(window))/(j+1);
