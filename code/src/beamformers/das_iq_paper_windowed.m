function [envelope_final_temp, pw_final] = das_iq_paper_windowed(scan,pht,dataset,pw_indices) %, path_reconstruted_img, path_image
    ...
    for f=1:length(pw_indices)
        ...
        for pw=pw_indices{f}
            ...
            envelope_temp_data = reshape(temp_data(:,f),[numel(scan.z_axis) numel(scan.x_axis)  1]);
            ...
            figure();
            imshow(window);
            % saveas(gcf, ['Results weighting strategy 2/simulation/MEAN-FILTER/Window-32 Uniform/TwoMinus/window_', num2str(j), '.jpg']);
            ...
            dynamic_range = 60;
            % filename1 = ['Results weighting strategy 2/simulation/MEAN-FILTER/Window-32 Uniform/TwoMinus/iteration', num2str(j), '.jpg'];
        ...
        if mean(contrast_reg-con_init)>0
            con_init = contrast_reg;
            envelope_final_temp = abs(reg_image);
            pw_final = pw_indices{f};
        end
    end

    % dynamic_range = 60;
    % envelope_final_temp = abs(reg_image);
    % % windowed_path_jpg = ['Results weighting strategy 2/simulation/MEAN-FILTER/Window-32 Uniform/TwoMinus/windowed_15', '.jpg'];
    % close(wb);
    % reg_us = us_image('DAS-IQ beamforming');
    % reg_us.author = 'Alfonso Rodriguez-Molares <alfonso.r.molares@ntnu.no>';
    % reg_us.affiliation = 'Norwegian University of Science and Technology (NTNU)';
    ...
    % disp(mean(cnr_value));
    % % windowed_path = ['Results weighting strategy 2/simulation/MEAN-FILTER/Window-32 Uniform/TwoMinus/windowed_15', '.hdf5'];
    ...
    % 
    % % unwindowed_path_jpg = ['Results weighting strategy 2/simulation/MEAN-FILTER/Window-32 Uniform/TwoMinus/unwindowed_15', '.jpg'];
    ...
    % unwindowed_path = ['Results weighting strategy 2/simulation/MEAN-FILTER/Window-32 Uniform/TwoMinus/unwindowed_15', '.hdf5'];
