function image = das_iq_trial_apodization(scan,dataset,arr) %, path_reconstruted_img, path_image
    ...
    % Define adaptive apodization window function
    z_max = max(scan.z); % Max depth
    adaptive_weight = 1 - exp(-scan.z / (0.2 * z_max)); % Weight increases with depth
    adaptive_weight = adaptive_weight / max(adaptive_weight); % Normalize to [0,1]
    ...
    % Apply adaptive weighting to receive apodization
    receive_apodization = tools.apodization(rx_aperture_distance, rx_aperture * ones(1, dataset.channels), 'tukey50');
    receive_apodization = receive_apodization .* adaptive_weight; % Scale by depth-dependent weight
    receive_apodization = max(receive_apodization, 0.1); % Ensure minimum apodization
