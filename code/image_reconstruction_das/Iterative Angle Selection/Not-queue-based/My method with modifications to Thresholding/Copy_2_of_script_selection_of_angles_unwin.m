...
beamform_start = das_iq_one_image(scan, dataset, pw_indices{1});
...
con_angle = [mean(tools.contrast_score(scan,pht,reg_us,length(pw_indices{1})))];

for thresh=j(2:end)
    pw_indices{1} = (1+dataset.firings)/2;
    beamform_curr = beamform_start;
    disp(['Starting image reconstruction from ',acquisition,' for ',phantom,' using ',data,' dataset'])
    [beamform_curr, pw_indices, thresh] = Copy_of_selection_angles_seq_unwin(scan,dataset,pw_indices,(thresh-1),...
    ...
    con_angle = [con_angle, mean(tools.contrast_score(scan,pht,reg_us,length(pw_indices{1})))];
end

figure(); plot(j, con_angle, 'LineWidth', 3); xlabel('Angle limit'); ylabel('CNR');
saveas(gcf, 'unwindowed_curve.png');
save('25_unwindowed.mat');
