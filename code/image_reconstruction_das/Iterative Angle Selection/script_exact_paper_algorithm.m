...
% end
beamform_prev = zeros([numel(scan.z_axis) numel(scan.x_axis)]);
...
queue = {};
queue{end+1} = [1, dataset.firings];


disp(['Starting image reconstruction from ',acquisition,' for ',phantom,' using ',data,' dataset'])
while ~isempty(queue)
    [beamform_curr, pw_indices, queue] = selection_exact_paper_angles_met1(scan,dataset,pht,pw_indices,0.90,...
        queue, beamform_curr);
...
saveas(gcf,'Results/Sampling Strategies/Exact_paper/Method1/unwindowed/experiments/queue_final_result.jpg');
reg_us.write_file('Results/Sampling Strategies/Exact_paper/Method1/unwindowed/experiments/queue_final_result.hdf5');
tools.exec_evaluation_contrast_speckle(path_scan,path_pht, ...
    'Results/Sampling Strategies/Exact_paper/Method1/unwindowed/experiments/queue_final_result.hdf5', ...
    flag_simu,flag_display, ...
    'Results/Sampling Strategies/Exact_paper/Method1/unwindowed/experiments/queue_final_result.txt');
