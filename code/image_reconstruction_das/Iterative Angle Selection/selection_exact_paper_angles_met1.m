function [beamform_curr, pw_indices, queue] = selection_exact_paper_angles_met1(scan,dataset,pht,pw_indices,thresh,queue,beamform_curr)
    ...
    beamform_now = das_iq_paper_unwindowed(scan,dataset,pw_indices{4});
    ...
    if max(abs(beamform_now(:)))>=1
        beamform_now1 = beamform_now/max(abs(beamform_now(:)));
    else
        beamform_now1 = beamform_now;
    end
