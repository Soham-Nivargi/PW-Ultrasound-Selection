...
while ~isempty(queue)
    [beamform_curr, pw_indices, queue] = selection_angles_seq_unwin(scan,dataset,pw_indices,0.99,...
        queue, beamform_curr);
    queue(1) = [];  
end
...
saveas(gcf,'final_result.jpg');
reg_us.write_file('final_result.hdf5');
tools.exec_evaluation_contrast_speckle(path_scan,path_pht,'final_result.hdf5', ...
    flag_simu,flag_display,'final_result.txt');
