...
while ~isempty(queue)
    [beamform_curr, pw_indices, queue] = selection_angles_seq_unwin(scan,dataset,pw_indices,thresh,...
        queue, beamform_curr);
    queue(1) = [];  
end
...
saveas(gcf,'Results/Simulation/Threshold limit/unwindowed_final_result.jpg');
reg_us.write_file('Results/Simulation/Threshold limit/unwindowed_final_result.hdf5');
tools.exec_evaluation_contrast_speckle(path_scan,path_pht,'Results/Simulation/Threshold limit/unwindowed_final_result.hdf5', ...
    flag_simu,flag_display,'Results/Simulation/Threshold limit/unwindowed_final_result.txt');
save('Results/Simulation/Threshold limit/variables_unwindowed.mat', 'pw_indices');
