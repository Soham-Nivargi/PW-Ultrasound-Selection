...
con_angle = [mean(tools.contrast_score(scan,pht,reg_us,length(pw_indice)))];
...
for k=j(2:end)
    ...
    con_angle = [con_angle, mean(tools.contrast_score(scan,pht,reg_us,length(pw_indice)))];
end
...
% save('Results/Simulation/variables_unwindowed.mat');
