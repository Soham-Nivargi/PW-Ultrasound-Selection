...
% path_reconstruted_img = ['Results/greedy_simulation/new_window/greedy_selection_',phantom,'_',acqui,'_img_from_',data,'_K_',num2str(K),'.hdf5'];
...
image = greedy_selection_window(phantom_type, acquisition_type,...
                      num2str(flag_simu), num2str(flag_display), dataset, scan, pht, K, init_set);


%-- Show the corresponding beamformed images
dynamic_range = 60;
image.show(dynamic_range);
saveas(gcf, ['Results/simulation/Window-16 Blurred/final_result_windowed.jpg']);


%-- Save results
image.write_file(['Results/simulation/Window-16 Blurred/final_result_windowed.hdf5']);

tools.exec_evaluation_contrast_speckle(path_scan,path_phantom,['Results/simulation/Window-16 Blurred/final_result_windowed.hdf5'], ...
    num2str(flag_simu),num2str(flag_display), ...
    ['Results/simulation/Window-16 Blurred/final_result_windowed.txt']);
