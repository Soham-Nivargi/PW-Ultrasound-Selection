...
beamform_start = das_iq_one_image(scan, dataset, pw_indices);
...
reg_us.data = abs(beamform_start);
...
j=linspace(1,25,13);
con_angle = [mean(tools.contrast_score(scan,pht,reg_us,length(pw_indices)))];

% disp(['Starting image reconstruction from ',acquisition,' for ',phantom,' using ',data,' dataset'])
for thresh=j(2:end)
    pw_indices = (1+dataset.firings)/2;
    beamform_curr = beamform_start;
    % disp(['Starting image reconstruction from ',acquisition,' for ',phantom,' using ',data,' dataset'])
    [beamform_curr, pw_indices, thresh] = Copy_of_selection_angles_seq_win(scan,dataset,pht,pw_indices,(thresh-1), ...
            1, (1+dataset.firings)/2, dataset.firings, beamform_curr);
    % queue(1) = [];  

    reg_us = das_iq_windowed(scan,dataset,{pw_indices}, path_scan, path_pht, flag_simu, flag_display);

    con_angle = [con_angle, mean(tools.contrast_score(scan,pht,reg_us,length(pw_indices)))];
end
    
figure(); plot(j, con_angle, 'LineWidth', 3); xlabel('Angle limit'); ylabel('CNR');
saveas(gcf, 'windowed_curve.png');
save('25_windowed.mat');
...
% saveas(gcf,'Results/Simulation/final_result_windowed.jpg');
% reg_us.write_file('Results/Simulation/final_result_windowed.hdf5');
% tools.exec_evaluation_contrast_speckle(path_scan,path_pht,'Results/Simulation/final_result_windowed.hdf5', ...
%     flag_simu,flag_display,'Results/Simulation/final_result_windowed.txt');
% save('Results/Simulation/variables_windowed.mat');
