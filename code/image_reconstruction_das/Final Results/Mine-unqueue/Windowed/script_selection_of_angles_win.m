...
% while ~isempty(queue)
[beamform_curr, pw_indices] = selection_angles_seq_win(scan,dataset,pht,pw_indices,thresh,...
        1, (1+dataset.firings)/2, dataset.firings, beamform_curr);
    % queue(1) = [];  % Remove first element
% end

pw_to_be_put = {pw_indices};
reg_us = das_iq_windowed(scan,pht,dataset,pw_to_be_put);
...
save('Results/simulation/Threshold Limit/Window-16 Blurred/variables_windowed.mat', 'pw_indices');
