...
pw_indices = [1, dataset.firings];%-- dataset.firings corresponding to the total number of emitted steered plane waves
flag_simu = num2str(flag_simu);
flag_display = num2str(flag_display);
beamform_start = das_iq_paper_unwindowed(scan, dataset, pw_indices);
...
for thresh=j(2:end)
    ...
    [beamform_curr, pw_indices] = Copy_of_selection_angles_seq_unwin(scan,dataset,pw_indices,...
            1, dataset.firings, beamform_curr, thresh);
    % queue(1) = [];  
...
saveas(gcf, 'Results/Simulation/unwindowed_curve.png');
save('Results/Simulation/25_unwindowed.mat');
