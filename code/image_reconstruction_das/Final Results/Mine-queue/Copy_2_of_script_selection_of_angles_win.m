...
for k=j(2:end)
    % disp(['Starting image reconstruction from ',acquisition,' for ',phantom,' using ',data,' dataset'])
    [beamform_curr, pw_indices, queue] = Copy_of_selection_angles_seq_win(scan,dataset,pht,pw_indices,...
            queue, beamform_curr);
    queue(1) = [];  
...
saveas(gcf, 'Results/Simulation/windowed_curve.png');
save('Results/Simulation/25_windowed.mat');
